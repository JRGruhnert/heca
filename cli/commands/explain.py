...
class ExplainScript:
    def __init__(self, config: ExplainManagerConfig):
        ...
        self.agent = HoopGNSkill(config.skill)
        ...
        match = self.object_pattern.search(
            self.config.skill.network.checkpoint_path or ""
        )
        if match:
    ...
    def is_different_on_start(self, current: TDProperties, goal: TDProperties):
    ...
    def run_batch(self):
        ...
        for i in trange(
            self.config.skill.batch_size, desc="Collecting samples for explanation"
        ):
        ...
        self.plot.create(
            ...
            True,
            True,
            f"{self.storage.agent_saving_path(self.config.skill.network.label)}/plots/{self.trained_object}_s_{self.current_object}.png",
        )
