...
from heca.misc.td import TDCamRecord, TDSceneReferences


class ReferenceSelector(Configurable):
    ...
    def load_image(self):
        ...
        recordings = self.scene.sample_td_images()
        ...
        self.point = None
        self.marker = None
        self.highlight_references(record)

    def highlight_references(self, record: TDCamRecord):
        assert self.current_tuple is not None
        # Code for highlighting references
        self.scene.extractor.prepare_references(self.result)
        td_cam_references = self.result.cams.get(self.current_tuple[0])
        if td_cam_references is None:
            return
        print(td_cam_references.keys())
        print(td_cam_references.pose_refs.keys())
        print(td_cam_references.entity_states.keys())
...
selector_cfg = ReferenceSelector.Config(
    scene=CalvinScene.Query(),
)
selector = ReferenceSelector.create(selector_cfg)
selector.run()
