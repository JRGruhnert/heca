# NOTE: Copied from OGBENCH https://github.com/seohongpark/ogbench
...
def main(_):
    ...
    # Collect data — stream directly to HDF5 to avoid holding the full
    # dataset in RAM. We keep only the current episode buffered in memory
    # so that unhealthy episodes can be rolled back cheaply.
    total_steps = 0
    total_train_steps = 0
    ...
    h5_path = FLAGS.save_path.replace(".npz", ".h5")
    h5_file = h5py.File(h5_path, "w")
    h5_datasets: dict = {}
    written_steps = 0  # steps flushed to HDF5 so far
    ...
    def _flush_episode(ep_buf: dict):
        ...
        for k, rows in ep_buf.items():
            arr = np.array(rows)
            if k not in h5_datasets:
                maxshape = (None,) + arr.shape[1:]
                h5_datasets[k] = h5_file.create_dataset(
    ...
    for ep_idx in trange(num_train_episodes + num_val_episodes):
        ...
        # Only the current episode is buffered in memory; the rest lives on disk.
        ep_buf: dict = defaultdict(list)
        while True:
            ob, info = env.reset()
            ...
            while not done:
                ...
                if agent.done:
                    ...
                    agent_ob, agent_info = env.unwrapped.set_new_target(p_stack=p_stack)  # type: ignore
                    agent = agents[agent_info["privileged/target_task"]]
                    agent.reset(agent_ob, agent_info)

                ep_buf["observations"].append(ob)
                ep_buf["actions"].append(action)
                ep_buf["terminals"].append(done)
                ep_buf["qpos"].append(info["prev_qpos"])
                ep_buf["qvel"].append(info["prev_qvel"])
                if has_button_states:
                    ep_buf["button_states"].append(info["prev_button_states"])
            ...
            if "scene" in FLAGS.env_name:
                ...
                if is_healthy:
                    _flush_episode(ep_buf)  # commit to disk
                    break
                else:
                    print("Unhealthy episode, retrying...", flush=True)
                    ep_buf = defaultdict(list)  # discard; nothing written to disk yet
            else:
                _flush_episode(ep_buf)  # commit to disk
                break
    ...
    print(
        f"Train steps: {total_train_steps}  Val steps: {written_steps - total_train_steps}"
    )
    print("Done.")
