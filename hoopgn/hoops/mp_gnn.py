...
class MPGnn(MPNetwork):
    ...
    def forward(self, x: TDScene, y: TDScene) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def s_s_attr(
        self, x: TDScene, pad: bool = True, sparse: bool = False
    ) -> torch.Tensor:
        dist_matrix = self.skill_state_distances(x, pad, sparse)  # [T, S, 1 or 2]
    ...
    def state_skill_attr_weighted_sparse(
        self, x: TDScene, pad: bool = True
    ) -> torch.Tensor:
        dist_matrix = self.skill_state_distances(x, pad)  # [T, S, 2]
        # Now safely get edge attributes for (task, state) pairs: [E, 2]
        edge_attr = dist_matrix[
            self.state_skill_full[1], self.state_skill_full[0]
        ]  # [E, 2]
