...
@dataclass
class PositionsSkill3DConfig(TrainingsPlotConfig):
    name: str = "Positions3D"
...
class PositionsSkill3D(TrainingsPlot):
    def __init__(self, config: PositionsSkill3DConfig):
    ...
    def show_objects(self, alpha: float = 0.75):
        for start_pos, start_quat, end_pos, end_quat, diff, solved in zip(
            ...
            color = self.config.color_map.get("object", "m")
    ...
    def show_ellipsoid(self, alpha: float = 0.25):
        for key, axis in self.axes.items():
            ...
            z = xyz[:, :, 2] + mean[2]
            axis.plot_surface(
                x, y, z, color=self.config.color_map.get("ellipsoid", "k"), alpha=alpha
            )
    ...
    def show_edges(self, alpha: float = 0.5):
        for start_pos, start_quat, end_pos, end_quat, diff, solved in zip(
            ...
            axis.plot(
                ...
                color=self.config.color_map["solved" if solved else "unsolved"],
                alpha=alpha,
            )
        ...
        self.fig.legend(
            handles=[
                Line2D(
                    ...
                    color="w",
                    label="Object",
                    markerfacecolor=self.config.color_map.get("object", "m"),
                    markersize=10,
                ),
                Line2D(
                    [0],
                    [0],
                    color=self.config.color_map.get("solved", "g"),
                    label="Solved",
                ),
                Line2D(
                    [0],
                    [0],
                    color=self.config.color_map.get("unsolved", "r"),
                    label="Unsolved",
                ),
            ],
            loc="upper right",
