...
class HoopGNSkillConfig(SkillConfig):
...
class HoopGNSkill(Skill):
    def __init__(self, config: HoopGNSkillConfig):
    ...
    def learn(self) -> bool:
        ...
        if self.config.lr_annealing:
            lr = self.config.learning_rate * (1.0 - self.buffer.progress)
            for param_group in self.optimizer.param_groups:
                param_group["lr"] = lr

        self.update_network()

        if self.buffer.epoch % self.config.saving_freq == 0 and self.buffer.epoch != 0:
            self.save(highscore=False)

        self.buffer.save(self.config.label)
    ...
    def save(self, highscore: bool):
        ...
        ckpt_path = (
            self.config.saving_path + tag + "_epoch{}.pth".format(self.buffer.epoch)
        )
