from dataclasses import dataclass

from heca.data.entity import Entity


class EEEntity(Entity):
    BASE_LOGSTD = -5.0

    @dataclass(kw_only=True)
    class Config(Entity.Config):
        label: str = "ee"
        states: list[str] = ["open", "closed"]
        question: str = "Is the gripper of the robot arm:"
        answers: list[str] = ["open", "closed"]

    @classmethod
    def gnn_format(cls, raw, n_states, logit_confidence=10.0, base_logstd=None):
        if base_logstd is None:
            base_logstd = cls.BASE_LOGSTD
        return super().gnn_format(raw, n_states, logit_confidence, base_logstd)
