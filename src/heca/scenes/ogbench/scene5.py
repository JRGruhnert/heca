...
class OGScene5(OGScene):
    ...
    def to_dc_scene(self, obs: dict) -> DCScene:
        ...
        for entity in self.entities:
            e_pos = obs[f"heca_{entity.cfg.label}_pos"]
            e_rot = obs[f"heca_{entity.cfg.label}_rot"]
            # e_rot = np.array([wxyz[1], wxyz[2], wxyz[3], wxyz[0]], dtype=np.float32)
            e_ste = np.atleast_1d(obs[f"heca_{entity.cfg.label}_ste"])
