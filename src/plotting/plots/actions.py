...
def plot(collection: RunDataCollection):
    for nt in [NT_GNN, NT_MLP]:
        for index, domain in enumerate([
            ...
            g_d_success_len = run_g_d.stats["batch_stats"][-1]["mean_success_episode_length"]
            g_t_success_len = run_g_t.stats["batch_stats"][-1]["mean_success_episode_length"]
            b_d_success_len = run_b_d.stats["batch_stats"][-1]["mean_success_episode_length"]
            b_t_success_len = run_b_t.stats["batch_stats"][-1]["mean_success_episode_length"]

            g_d_failure_len = run_g_d.stats["batch_stats"][-1]["mean_failure_episode_length"]
            g_t_failure_len = run_g_t.stats["batch_stats"][-1]["mean_failure_episode_length"]
            b_d_failure_len = run_b_d.stats["batch_stats"][-1]["mean_failure_episode_length"]
            b_t_failure_len = run_b_t.stats["batch_stats"][-1]["mean_failure_episode_length"]
            ...
            legend_text = (
                ...
                g_t_sr, 100*(g_d_sr-g_t_sr)/max(1e-6, g_t_sr), b_t_sr, 100*(b_d_sr-b_t_sr)/max(1e-6, b_t_sr),
                g_t_len, 100*(g_d_len-g_t_len)/max(1e-6, g_t_len), b_t_len, 100*(b_d_len-b_t_len)/max(1e-6, b_t_len),
                g_t_success_len, 100*(g_d_success_len-g_t_success_len)/max(1e-6, g_t_success_len), b_t_success_len, 100*(b_d_success_len-b_t_success_len)/max(1e-6, b_t_success_len),
                g_t_failure_len, 100*(g_d_failure_len-g_t_failure_len)/max(1e-6, g_t_failure_len), b_t_failure_len, 100*(b_d_failure_len-b_t_failure_len)/max(1e-6, b_t_failure_len)
            )

            # Add legend box to plot
            ax.text(
                0.45, 0.85, legend_text,
            ...
            ax.set_title(f"Delta Action Distribution: {dom[index][0]} vs {dom[index][1]} (trained on {dom[index][0]})")
            ax.legend()
